...
typedef StateTransformer<State> = State Function(State newState);

class EmitterMiddleware<State> implements Emitter<State> {
  final Emitter<State> _originalEmitter;
  final StateTransformer<State> _transformer;

  EmitterMiddleware(this._originalEmitter, this._transformer);

  @override
  void call(State state) {
    final transformedState = _transformer(state);

    return _originalEmitter(transformedState);
  }
